...
- **Python**: We use [`ruff`](https://docs.astral.sh/ruff/) for formatting and linting. Run `uv run ruff check .` to check your code and `uv run ruff format .` to auto-fix formatting issues before committing. Pre-commit hooks enforce these checks.
- **Markdown**: Follow [markdownlint](https://github.com/DavidAnson/markdownlint) rules (see `.markdownlint.json`). Run `uv run markdownlint docs/` to check.
